# from agent.utils import _serialize_value, export_csv
...
def get_repayment_summary_node(
    ...
    try:
        ...
        if total == 0:
            result = {
                ...
                **({"loan_id": params.loan_id} if params.loan_id else {}),
                ...
                "message": (
                    f"No past payment records found for Customer ID: {params.customer_id}"
                    + (f" and Loan ID: {params.loan_id}" if params.loan_id else "")
                ),
                "csv_path": None,
                "dataset_id": None,
            }
        ...
        avg_dpd = round(sum(r["dpd_days"] or 0 for r in past) / total, 1)

        # trend: compare last 3 vs previous 3 repayments
        sorted_past = sorted(past, key=lambda r: r["due_date"])
        recent = sorted_past[-3:] if len(sorted_past) >= 3 else sorted_past
        earlier = sorted_past[-6:-3] if len(sorted_past) >= 6 else []
        ...
        print("==========================================================")
        print("DEBUG: in get_repayment_summary_node:")
        print(f"DEBUG: cache_key: {cache_key}")
        print(f"DEBUG: customer: {customer['full_name']}")
        ...
        print("==========================================================")
        print("DEBUG: in get_repayment_summary_node exception occurred:")
