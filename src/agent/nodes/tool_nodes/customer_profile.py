...
def get_customer_profile_node(
    ...
    try:
        ...
        cur.execute("""
            ...
            FROM customers
            WHERE customer_id = %s
        """, (params.customer_id,)) #The trailing comma in (params.customer_id,) makes it a tuple — required by psycopg2 even for a single value.
        ...
        for k, v in customer.items():
            if isinstance(v, Decimal): customer[k] = float(v)
            elif isinstance(v, date):  customer[k] = v.isoformat()
